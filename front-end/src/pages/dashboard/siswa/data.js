...
const fetchPelajaranSaya = async () => {
  try {
    const url = "dashboard/stat-siswa";
    const response = await fetchWithAuth(url, { method: "GET" });
    return response;
  } catch (error) {
    console.error("Error fetching user:", error);
...
const fetchMateriSiswaById = async (id) => {
  const response = await fetchWithAuth(
    `${API_ENDPOINTS.MATERI_SISWA}${id}/edit`,
    {
      method: "GET",
    }
  );
...
const usePelajaranSaya = () => {
  return useQuery({
    queryKey: ["daftar-pelajaran-saya"],
    queryFn: fetchPelajaranSaya,
    ...QUERY_CONFIG,
  });  
};
