...
export default function QuestionList({ questions }) {
  ...
  const { aktifkanKonfirmasi } = useKonfirmasi();
  const [itemToDelete, setItemToDelete] = useState(null);
  const { notify } = useNotification();

  const { apakahHapus } = useContext(UserContext);

  useEffect(() => {
    const deleteMateri = async () => {
      if (itemToDelete && apakahHapus) {
        try {
          const url = `${
            import.meta.env.VITE_API
          }soal/hapus-soal/${itemToDelete}`;
          ...
          //   queryKey: ["materi"],
          // });
          aktifkanKonfirmasi(false);
          setItemToDelete(null);

          setFields(fields.filter((field) => field.id !== itemToDelete));

          notify({
            type: "success",
            message: response.detail || "Materi berhasil dihapus",
          });
    ...
    deleteMateri();
  }, [aktifkanKonfirmasi, itemToDelete, fields, apakahHapus, notify]);

  const handleDelete = useCallback(
    (row) => {
      aktifkanKonfirmasi(true);
      setItemToDelete(row);
    },
    [aktifkanKonfirmasi, setItemToDelete]
  );
              ...
              {questions.map((field) => (
                ...
                >
                  ...
                  <div className="absolute top-2 right-2 flex gap-2">
                    ...
                    <Link to={`/soal/edit/${field.id}`} className="cursor-pointer text-indigo-500 hover:text-indigo-700  opacity-0 group-hover:opacity-100 transition-opacity">edit</Link>
                  </div>
