...
class PDFQuestionExtractor:
    ...
    def extract_simple_tables_from_text(self, pdf_path: str, page_num: int) -> List[Dict]:
        """Ekstrak tabel sederhana dari teks menggunakan pattern"""
        tables = []
        try:
            doc = fitz.open(pdf_path)
            page = doc[page_num]
            ...
            # Cari pattern yang mirip tabel (baris dengan separator)
            lines = text.split('\n')
            table_lines = []
            
            for line in lines:
                # Deteksi baris yang memiliki multiple kolom (separator: tab, spasi berlebih, |)
                if re.search(r'[\t\|]', line) or re.search(r'\s{3,}', line):
                    ...
                    if len(cols) >= 2:  # Minimal 2 kolom
                        table_lines.append(cols)
            
            # Jika ada baris tabel yang ditemukan
            if table_lines:
                ...
                for row in table_lines:
                    # Pad dengan string kosong jika kurang kolom
...
def main():
    ...
    pdf_path = "dokumen.pdf"  # Ganti dengan path file PDF Anda
